...
export async function runTests(affectedModules, config) {
    const testResults = [];
    
    // Load the appropriate plugin based on configuration
    const plugin = await loadPlugin(config.testFramework);
    
    for (const module of affectedModules) {
        const moduleConfig = config.modules.find(m => m.name === module);
...
async function loadPlugin(framework) {
    try {
